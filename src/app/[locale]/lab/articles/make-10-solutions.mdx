...
After running the script against all 10,000 combinations, the solver managed to crack 9,981 of them.

However, there are **19 numbers** that my code could not solve.

It is entirely possible that solutions exist for these, but they likely require an operator I haven't implemented or a search depth I wasn't willing to compute. If you see a solution for any of these (preserving the digit order), I’d be interested to hear it.

**The Unsolved List:** 0607, 0670, 0671, 0706, 0716, 0760, 0761, 1670, 1671, 1706, 1716, 1760, 1761, 6070, 6071, 6170, 6171, 7060, 7061.

Below is the full, exported list of solutions that the solver produced for every 4-digit input. The interactive list includes the simplest found expression for each solvable input; the unsolved inputs above just show "NoSolution".
